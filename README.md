...
Model capabilities are explicit. Image attachment and native web search controls only work when the selected model is marked as supporting them. Generic OpenAI-compatible services are not assumed to implement a compatible search extension.
...
- Images are copied to private storage and resized to a maximum edge of 2048 pixels when necessary.
